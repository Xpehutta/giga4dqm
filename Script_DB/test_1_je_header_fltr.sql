...
*/
drop FUNCTION if exists s_grnplm_as_t_didsd_nnn_db_tmd.sp_spt_je_header_fltr(int8, text);
CREATE OR REPLACE FUNCTION s_grnplm_as_t_didsd_nnn_db_tmd.sp_spt_je_header_fltr(in in_workflow_run_id int8, in in_info_system_inst_cd text, out return_int_return_code int4)
	RETURNS int4
	LANGUAGE plpgsql
	VOLATILE
AS $$
