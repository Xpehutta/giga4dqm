...
*/
drop FUNCTION if exists s_grnplm_as_t_didsd_nnn_db_tmd.sp_spt_je_header_fltr_skew_data_separate(int8, int8, text);
CREATE OR REPLACE FUNCTION s_grnplm_as_t_didsd_nnn_db_tmd.sp_spt_je_header_fltr_skew_data_separate(in in_workflow_run_id int8,in in_data_wf int8, in in_info_system_inst_cd text, out return_int_return_code int4)
	RETURNS int4
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
v_sql = case when in_info_system_inst_cd = '000004'
	...
	select je_header_id,
        ...
        je_line_cred_ind,
		  je_type_id,
        src_system_type_id
  from s_grnplm_as_t_didsd_nnn_db_tmd.t_je_line'||in_data_wf::text||' ln
  where 
  ln.deleted_flag = ''N''
    and set_of_books_cd in ({{books_cd}})
    and info_system_inst_cd = ''{{in_info_system_inst_cd}}''
